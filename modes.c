...
static void buf_yank(const char *data, int len)
{
    ...
    if (pb >= 'a' && pb <= 'z') {
        ...
        if (E.nbuf_data[idx])
            memcpy(nd, E.nbuf_data[idx], E.nbuf_len[idx]);
        memcpy(nd + E.nbuf_len[idx], data, len);
        free(E.nbuf_data[idx]);
        E.nbuf_data[idx] = nd;
        E.nbuf_len[idx]  = newlen;
    } else {
        free(E.yank_data);
...
static void buf_get(char **data_out, int *len_out)
{
    ...
    if (pb >= 'a' && pb <= 'z') {
        int idx   = pb - 'a';
        *data_out = E.nbuf_data[idx];
        *len_out  = E.nbuf_len[idx];
    } else if (pb >= 'A' && pb <= 'Z') {
        int idx   = pb - 'A';
        *data_out = E.nbuf_data[idx];
        *len_out  = E.nbuf_len[idx];
...
void process_normal(int c)
{
    ...
    case KEY_DEL: {
        if (E.lines[E.cy].len > 0 && E.cx < E.lines[E.cy].len) {
            save_undo();
            line_delete_char(&E.lines[E.cy], E.cx);
            E.dirty = 1;
            clamp_cursor();
    ...
    case 'D': {
        if (E.lines[E.cy].len > 0) {
            save_undo();
            buf_yank(E.lines[E.cy].data + E.cx,
    ...
    case 'r':
        E.wait_state = WAIT_REPLACE_CHAR;
        E.wait_count = cnt;
        break;
    ...
    case 0x07: {
        ...
        int p = E.nlines > 0
                ? (int)((long)(E.cy + 1) * 100 / E.nlines) : 0;
        snprintf(E.statusmsg, sizeof(E.statusmsg),
                 "\"%s\"%s  %d/%dL  %d%%",
                 fname, E.dirty ? " [Modified]" : "",
                 E.cy + 1, E.nlines, p);
        break;
    }
...
{
    int   sw  = E.shiftwidth > 0 ? E.shiftwidth : 8;
    Line *l   = &E.lines[E.cy];
    int   col = col_of_cx();
    int   tgt = ((col - 1) / sw) * sw;
    if (tgt < 0) tgt = 0;
...
static void ins_ctrl_w(void)
{
    Line *l = &E.lines[E.cy];
    if (E.cx == 0) return;
    while (E.cx > 0 && is_blank(l->data[E.cx - 1])) {
        E.cx--;
        line_delete_char(l, E.cx);
    }
    while (E.cx > 0 && !is_blank(l->data[E.cx - 1])) {
